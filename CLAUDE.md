...
### Core Apps
- `accounts`: Custom `User` model, OTP-based login (currently on-screen, designed for SMS), and role management.
- `catalog`: Service taxonomy (`ServiceCategory` $\rightarrow$ `Service`).
- `bookings`: Booking lifecycle, individual bookings, and institutional `BulkServiceRequest` flows.
- `workers`: Manages the cooperative hierarchy (`Federation` $\rightarrow$ `Society` $\rightarrow$ `WorkerProfile`).
- `dashboard`: Role-specific administrative and moderation interfaces.
...
- **Matching Engine** (`bookings/services.py`): Logic to find best workers based on distance, ratings, and work distribution fairness.
- **Geo-Service** (`workers/geo.py`): Centralized logic for address geocoding and road-distance calculations.
...
- **Geocoding**: Google Maps Distance Matrix API for road-distance and ETA-based worker matching.
...
- **Status-Driven State Machine**: `Booking` model implements a strict `STATUS_FLOW` and `WORKER_ACTION_MAP` to ensure valid state transitions.
